...
python run.py --output_dir=./saved_models --model_type=roberta --config_name=microsoft/codebert-base --model_name_or_path=microsoft/codebert-base --tokenizer_name=roberta-base --do_test --train_data_file=../dataset/train.txt --eval_data_file=../dataset/valid.txt --test_data_file=../dataset/test.txt --epoch 2 --block_size 400 --train_batch_size 16 --eval_batch_size 32 --learning_rate 5e-5 --max_grad_norm 1.0 --evaluate_during_training  --seed 123456
...
def get_example(item):
    ...
    if url1 in cache:
        code1=cache[str(url1)].copy()
    else:
        code=' '.join(url_to_code[str(url1)].split())
        code1=tokenizer.tokenize(code)

    if url2 in cache:
        code2=cache[str(url2)].copy()
    else:
        code=' '.join(url_to_code[str(url2)].split())
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512,pool=None):
        ...
        with open(r"C:\git\876\code_segments.json", "r") as f:
            url_to_code = json.load(f)
        
        if "train" in postfix:
            with open(r"C:\git\876\train_pairs.json", "r") as f:
                l = json.load(f)
        else:
            with open(r"C:\git\876\test_pairs.json", "r") as f:
                l = json.load(f)

        data=[]
        cache={}
        for line in l:
            url1,url2,label= line["func1id"], line["func2id"], line["clonetype"]
            
            data.append((url1,url2,int(label),tokenizer, args,cache,url_to_code))
...
def evaluate(args, model, tokenizer, prefix="",pool=None,eval_when_training=False):
    ...
    for i in range(1,100):
        ...
        y_preds = [np.argmax(l) for l in logits]
        # print(y_preds)
...
def test(args, model, tokenizer, prefix="",pool=None,best_threshold=0):
    ...
    for batch in eval_dataloader:
        ...
        nb_eval_steps += 1
    # print("before", len(logits))
    # logits_=np.concatenate(logits,0)
    # print("after ", len(logits_))
    # print("trues", len(y_trues))
    print(y_trues[:100])
    # print(logits)
    logits = [np.argmax(l) for l in logits]
    print(logits[:100])
    cm = confusion_matrix(y_trues, logits)
    cm_display = ConfusionMatrixDisplay(confusion_matrix = cm, display_labels = [0, 1, 2, 3, 4])
    cm_display.plot()
    plt.savefig("cm.png")

    output_binary = [0 if l == 0 else 1 for l in logits]
    true_binary = [0 if l == 0 else 1 for l in y_trues]
    tp = 0
    tn = 0
    for i in true_binary:
        if i == 0:
            tn += 1
        else:
            tp += 1
    ...
    op = 0
    on = 0
    for i in output_binary:
        if i == 0:
            on += 1
        else:
            op += 1
    print("Predicted Negative", on, "Predicted Positive", op)
    print(output_binary[:100])
    print(true_binary[:100])
    cm = confusion_matrix(true_binary, output_binary)
    ...
    output_type4 = [1 if l == 4 else 0 for l in logits]
    true_type4 = [1 if l == 4 else 0 for l in y_trues]
    cm = confusion_matrix(output_type4, true_type4)
    cm_display = ConfusionMatrixDisplay(confusion_matrix = cm, display_labels = [0, 4])
    cm_display.plot()
    plt.savefig("cm_type4.png")

    recall = recall_score(true_type4, output_type4)
    precision = precision_score(true_type4, output_type4)
    f1 = f1_score(true_type4, output_type4)
    print(f"{i}-type-4: recall = {recall}, precision = {precision}, f1 = {f1}")

    # plt.show()
    # y_preds=logits[:,1]>best_threshold
    with open(os.path.join(args.output_dir,"predictions.txt"),'w') as f:
        json.dump(dict(
            clone_pair=[[str(x.url1), str(x.url2)] for x in eval_dataset.examples],
            predictions=[int(x) for x in logits],
            y_trues=[int(x) for x in y_trues]
        ), f)

    recall = recall_score(y_trues, logits, average='samples')
    precision = precision_score(y_trues, logits, average='samples')
    f1 = f1_score(y_trues, logits, average='samples')

    print(f"samples-multiclass: recall = {recall}, precision = {precision}, f1 = {f1}")
            
    for i in ['micro', 'macro', 'samples', 'weighted', 'binary']:
        recall = recall_score(true_binary, output_binary, average=i)
        precision = precision_score(true_binary, output_binary, average=i)
        f1 = f1_score(true_binary, output_binary, average=i)

        print(f"{i}-binary: recall = {recall}, precision = {precision}, f1 = {f1}")
